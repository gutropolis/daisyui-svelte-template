...
<script lang="ts">
  ...
  import { bindHandlers } from '$lib/utils/HtmlEditorUtils';
  import Label from '$lib/components/form/Label.svelte';
  import {TINY_EDITOR_API_KEY} from '$lib/enums/setting';

  export let id: string = uuid('tinymce-svelte'); // default values
  export let inline: boolean | undefined = undefined;
  export let disabled: boolean = false;
  export let apiKey: string = TINY_EDITOR_API_KEY;
...
{:else}
  <textarea id={id} bind:this={element} style="visibility:hidden" {placeholder}></textarea>
{/if}
</div>
